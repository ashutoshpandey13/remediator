...
from agents.dockerfile_remediation import update_dockerfile


def main():
    """Main workflow for automated security remediation."""
    
    # Step 0: Clean up previous clone
    clone_dir = "cloned_repo"
    if os.path.exists(clone_dir):
        print(f"\n[bold yellow]Cleaning up previous clone: {clone_dir}[/bold yellow]")
        import shutil
        shutil.rmtree(clone_dir)
        print("✓ Previous clone removed\n")
    ...
    has_deployment = files["deployment_yaml"] is not None
    has_package_json = files["package_json"] is not None
    
    if not has_deployment and not has_package_json:
        print("[yellow]⚠ No deployment.yaml or package.json found[/yellow]")
        print("[yellow]⚠ Skipping remediation - only CVE scan results available[/yellow]")
        print(f"\n[bold cyan]Summary:[/bold cyan]")
        print(f"- Found {len(cves)} CVEs in container image")
        print(f"- Created {len(jira_tickets)} JIRA tickets for tracking")
        print(f"- Check reports/image_scan.json for detailed CVE information")
        print(f"\n[bold yellow]To enable full remediation, add these files to your repository:[/bold yellow]")
        print("  - deployment.yaml (for Kubernetes security fixes)")
        print("  - package.json (for dependency vulnerability fixes)")
        return
    ...
    if has_deployment:
        ...
        print(f"✓ Found deployment.yaml")
    else:
        print("[yellow]⚠ deployment.yaml not found - will skip K8s remediation[/yellow]")
    ...
    if has_package_json:
        ...
        print("[yellow]⚠ package.json not found - will skip dependency remediation[/yellow]")
    
    print("\nGenerating remediation with AI...")
    ...
    # Parse remediation based on what files we have
    if has_deployment and has_package_json:
        # Both files present - expect both in response
        ...
        # Only deployment.yaml
        fixed_yaml = remediation.replace("===FIXED_DEPLOYMENT===", "").strip()
        with open(files["deployment_yaml"], "w") as f:
            f.write(fixed_yaml)
        print(f"✓ Updated: {files['deployment_yaml']}")
        ...
        # Only package.json
        fixed_package_json = remediation.replace("===FIXED_PACKAGE_JSON===", "").strip()
        with open(files["package_json"], "w") as f:
            f.write(fixed_package_json)
        print(f"✓ Updated: {files['package_json']}")
    ...
    if has_deployment:
        ...
        all_valid = all_valid and yaml_valid
    ...
    # Step 10.5: Update Dockerfile for CVE fixes
    if files["dockerfile"] and len(cves) > 0:
        print("\n[bold green]═══ STEP 10.5: Updating Dockerfile for CVE Fixes ═══[/bold green]\n")
        
        print(f"Detected {len(cves)} CVEs in base image")
        print("Updating Dockerfile to use more secure base image...")
        
        dockerfile_updated = update_dockerfile(files["dockerfile"], cves)
        
        if dockerfile_updated:
            print(f"✓ Dockerfile updated: {files['dockerfile']}")
            print("✓ Base image will be updated to latest secure version")
        else:
            print("[yellow]⚠ Dockerfile update failed - continuing with current version[/yellow]")
    
    # COMMENTED OUT: Git operations - directly apply fixes and re-scan
    ...
    #     pr_url = None
    #
    # # Step 12.5: Auto-Merge PR
    # if pr_url:
    #     print("\n[bold cyan]═══ STEP 12.5: Auto-Merging Pull Request ═══[/bold cyan]\n")
    #
    #     try:
    ...
    #             print("\n[bold cyan]═══ STEP 12.6: Pulling Latest Changes ═══[/bold cyan]\n")
    #             main_branch = pull_latest_changes(repo_path)
    #             print(f"✓ Now on {main_branch} branch with latest changes")
    #
    #         else:
    #             print("[yellow]⚠ PR set to auto-merge when checks pass[/yellow]")
    #             print("[yellow]⚠ Continuing with current branch for testing[/yellow]")
    #
    #     except Exception as e:
    #         print(f"[yellow]⚠ Auto-merge failed: {e}[/yellow]")
    #         print("[yellow]⚠ PR created but not merged - please merge manually[/yellow]")
    
    print("\n[bold green]✓ Fixes applied locally - proceeding to rebuild and re-scan[/bold green]\n")
    
    # Step 13: Rebuild Image
    print("\n[bold blue]═══ STEP 13: Rebuilding Container Image ═══[/bold blue]\n")
    
    remediated_image = None
    build_result = None
    ...
    # Re-scan image
    fixed_cves = []
    if files["dockerfile"] and build_result and build_result.returncode == 0 and remediated_image:
